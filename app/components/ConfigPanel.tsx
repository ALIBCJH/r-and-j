...
function SectionLabel({ children, count }: { children: React.ReactNode; count?: string }) {
  ...
  return (
    <div className="flex items-center justify-between mb-3">
      ...
      {count && (
        <span className="text-[9px]" style={{ color: c.panelSubtle }}>
          {count}
        </span>
      )}
    </div>
...
export default function ConfigPanel({
  ...
  curtainsOpen, isNight, lightIntensity,
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pt-6 pb-5 shrink-0">
        ...
        <p className="text-[11px] mt-1 leading-relaxed" style={{ color: c.panelSubtle }}>
          Refine your architectural window treatments.
        </p>
        <Divider />
      </div>
      ...
      <div className="flex-1 overflow-y-auto dark-scroll px-6 pb-4 space-y-5">
        ...
        {/* TEXTILE PALETTE */}
        <section>
          <SectionLabel count={`${CURTAIN_COLORS.length} available`}>
            Textile Palette
          </SectionLabel>
          ...
          <div className="space-y-1">
            ...
            >
              <div
                ...
                style={{ background: c.panelIconBg }}
              >
                <FoldIcon />
              </div>
              <div className="flex-1 text-left">
                <div className="text-xs font-medium" style={{ color: c.panelItemText }}>
                  {FOLD_LABEL[fabric]}
                </div>
                <div className="text-[9px]" style={{ color: c.panelItemSub }}>
                  Draping style · tap to change
                </div>
              </div>
              <span style={{ color: c.panelSubtle }}><ChevronIcon /></span>
            </button>
            ...
            >
              ...
              {/* Toggle pill */}
              <div
                className="w-8 h-[18px] rounded-full flex items-center px-0.5 transition-colors duration-200 shrink-0"
              ...
              >
                <div
                  ...
                  style={{ transform: curtainsOpen ? 'translateX(14px)' : 'translateX(0)' }}
                />
              </div>
            </button>
          </div>
        ...
        <section>
          ...
          <div className="space-y-2 mb-4">
            <div className="flex items-center justify-between">
              <span
                ...
                style={{ color: c.panelItemSub }}
              >
                Light Intensity
              </span>
              <span className="text-[9px] font-medium tabular-nums" style={{ color: '#C9A84C' }}>
                {lightIntensity}%
              </span>
            </div>
            <input
              ...
              onChange={e => onLightIntensity(Number(e.target.value))}
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-md text-[9px] font-semibold tracking-[0.15em] uppercase transition-all duration-200"
              style={{
                background: !isNight ? '#C9A84C' : c.panelItemBg,
                color:      !isNight ? '#0E0E10' : c.panelItemSub,
              }}
            >
              <SunIcon />
              Day
              ...
              <div className="flex flex-wrap gap-1.5">
                {WALL_COLORS.map(({ name, value }) => (
                  <button
                    key={value}
                    title={name}
                    onClick={() => onWallColor(value)}
                    className="w-6 h-6 rounded-sm transition-all duration-150 hover:scale-110"
                    style={{
                      backgroundColor: value,
                      outline: wallColor === value ? '2px solid #C9A84C' : '2px solid transparent',
                      outlineOffset: '2px',
                    }}
                  />
                ))}
              ...
              <div className="flex flex-wrap gap-1.5">
                {FLOOR_COLORS.map(({ name, value }) => (
                  <button
                    key={value}
                    title={name}
                    onClick={() => onFloorColor(value)}
                    className="w-6 h-6 rounded-sm transition-all duration-150 hover:scale-110"
                    style={{
                      backgroundColor: value,
                      outline: floorColor === value ? '2px solid #C9A84C' : '2px solid transparent',
                      outlineOffset: '2px',
                    }}
                  />
                ))}
      ...
      >
        <button
          className="w-full py-3.5 text-[10px] font-semibold tracking-[0.22em] uppercase transition-opacity hover:opacity-90 active:opacity-75"
          style={{
